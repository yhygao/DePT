...
from target_dino import train_target_domain as train_target_dino
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    work_dir = os.getcwd()
    args.log_dir = os.path.join(work_dir, 'exp', f"{args.project}", f"{args.memo}")
    os.makedirs(args.log_dir, exist_ok=True)
    configure_logger(args.rank, args.log_dir+f"/{args.learn.random_seed}.txt")
    logging.info(
    ...
    args.model.num_classes = NUM_CLASSES[args.data.dataset]
            ...
            for tgt_domain in args.data.target_domains:
                ...
                if use_wandb(args):
                    wandb.init(
                        project=args.project if args.project else args.data.dataset,
                        group=args.memo,
                        job_type=f"{src_domain}_{tgt_domain}",
                        name=f"seed_{args.seed}",
                        config=dict(args),
                    )
                # main loop

                if args.target_algorithm == 'tent':
                    train_target_tent(args)
                elif args.target_algorithm == 'shot':
                    train_target_shot(args)

                elif args.target_algorithm == "adacontrast":
                    train_target_adacontrast(args)
                elif args.target_algorithm == 'dino':
                    train_target_dino(args)
                elif args.target_algorithm == 'dino_online':
                    train_target_dino_online(args)
                if use_wandb(args):
                    wandb.finish()
