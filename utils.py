...
from model.dept_dino import GaussianBlur

LOG_FORMAT = "[%(levelname)s] %(asctime)s %(filename)s:%(lineno)s %(message)s"
LOG_DATEFMT = "%Y-%m-%d %H:%M:%S"

NUM_CLASSES = {"domainnet-126": 126, "VISDA-C": 12, "imagenet-c": 1000}
...
class CustomDistributedDataParallel(DistributedDataParallel):
    ...
    def __setattr__(self, name, value):
        try:
            super(CustomDistributedDataParallel, self).__setattr__(name, value)
        except AttributeError:
...
def adjust_learning_rate(optimizer, progress, args):
    ...
    """
    if progress < args.learn.warmup_iters:
        decay = float(progress) / float(args.learn.warmup_iters)
    else:
        progress -= args.learn.warmup_iters
        full_progress = args.learn.full_progress - args.learn.warmup_iters
        if args.optim.lr_decay == 'cos':
            decay = 0.5 * (1.0 + math.cos(math.pi * progress / full_progress))
        elif args.optim.lr_decay == 'exp':
            decay = (1 + 10 * progress / full_progress) ** -0.75
        elif args.optim.lr_decay == 'step':
            decay = 1.0
